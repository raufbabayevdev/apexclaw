# Contributing to ApexClaw

Thanks for your interest in contributing to ApexClaw! This guide will help you get started.
...
**Background:** ApexClaw previously used `main` in some documentation and scripts, which caused 404 errors, broken CI refs, and contributor confusion (see [#2929](https://github.com/apexclaw-labs/apexclaw/issues/2929), [#3061](https://github.com/apexclaw-labs/apexclaw/issues/3061), [#3194](https://github.com/apexclaw-labs/apexclaw/pull/3194)). As of March 2026, all references have been corrected, stale branches cleaned up, and the `main` branch permanently deleted.
...
1. **Find an issue.** Look for issues labeled [`good first issue`](https://github.com/apexclaw-labs/apexclaw/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — these are scoped for newcomers and include context to get moving quickly.
...
4. **Start with Track A.** ApexClaw uses three [collaboration tracks](#collaboration-tracks-risk-based) (A/B/C) based on risk. First-time contributors should target **Track A** (docs, tests, chore) — these require lighter review and are the fastest path to a merged PR.
...
ApexClaw supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.apexclaw/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.apexclaw/.secret_key` with restricted permissions
    - Use `apexclaw onboard` for guided setup
...
3. Generic env vars (`APEXCLAW_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `APEXCLAW_PROVIDER` / `PROVIDER`
- `APEXCLAW_MODEL`
...
- `~/.apexclaw/.secret_key` or similar key files
...
- If identity-like wording is required, use ApexClaw-centric labels only (for example: `ApexClawAgent`, `ApexClawOperator`, `apexclaw_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use ApexClaw/project-native roles instead of personal or real-world identities.
...
ApexClaw's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use ApexClaw-native identifiers only (`ApexClawAgent`, `apexclaw_user`, `apexclaw_node`).
...
- **Good identity-like label**: `ApexClawAgent`, `apexclaw_user`, `apexclaw_node`
...
ApexClaw uses a forward-only schema versioning system for `config.toml`. This section
...
1. `crates/apexclaw-config/src/migration.rs` contains `V1Compat`, a wrapper struct
...
1. Bump `CURRENT_SCHEMA_VERSION` in `crates/apexclaw-config/src/migration.rs`.
...
### `apexclaw config migrate`

Users can run `apexclaw config migrate` to rewrite their on-disk `config.toml` to the
...
ApexClaw uses a `#[derive(Configurable)]` proc macro to automatically handle secret
...
```rust
use apexclaw_macros::Configurable;
...
- Includes the field in `apexclaw config list --secrets`
- Makes it settable via `apexclaw config set channels.your-channel.bot-token`
...
- [ ] Any required identity-like wording uses ApexClaw/project-native labels only
