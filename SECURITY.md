...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/apexclaw-labs/apexclaw/security/advisories/new)
...
ApexClaw implements defense-in-depth security:
...
ApexClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t apexclaw .
docker inspect --format='{{.Config.User}}' apexclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace apexclaw gateway
```
